...
blockHeader* blockHeader_getFooter(blockHeader* p){
    blockHeader* complement = (void*) p + (p->size - HSIZE);
    return complement;
}
...
blockHeader** freeList_getPrev(void* p){
    return p + HSIZE + sizeof(blockHeader*);
}
...
int my_malloc_init(){
    ...
    blockHeader* freeBlock = mem_sbrk(HSIZE + FREEHSIZE);
    if(freeBlock == NULL){
        return -1;
    }
...
void *my_malloc(size_t size){
    ...
    if(spaceLeft < 2 * HSIZE + FREEHSIZE){
        ...
        void* finalNewFreeBlock = endNewBlock + 2 * HSIZE + FREEHSIZE;
        ...
        if(finalNewFreeBlock > mem_heap_hi()){
	        ...
	        for(int i = 0; i < blockAllocCount; i++){
		        void* newFreeBlock = endNewBlock + blockSize;
		        finalNewFreeBlock = newFreeBlock + 2 * HSIZE + FREEHSIZE;

		        blockHeader* secAllocBlock = endNewBlock;
        ...
        blockHeader* currNext = endNewBlock;
...
void *my_realloc(void *oldptr, size_t size){
    ...
    if(t->size - 2 * HSIZE >= size){
        ...
        if(residue >= 2 * HSIZE + FREEHSIZE){
            blockHeader* newFreeBlock = (void*) t + newBlockSize;
    ...
    if(!nextBlock->valid && nextBlock->size + t->size >= size + 2 * HSIZE + FREEHSIZE){
        ...
        if(memoryExtend(oldptr + size + 2 * HSIZE + FREEHSIZE) == -1){
            return NULL;
        }
...
int my_check(){
    ...
    while(currBlock){
        ...
        if(isEpilogue(currBlock->size)){
            ...
            epilogueFound = 1;
            break;
        } else if(currBlock->size <= 0 || currBlock->size > mem_heap_hi() - mem_heap_lo()){
